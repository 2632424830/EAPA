...
class re_train_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        text_pseudo_label = torch.tensor(ann['text_pseudo_label'])
        image_pseudo_label = torch.tensor(ann['image_pseudo_label'])

        ## if no need label, set value to zero or others:
        # label = 0
        # return image, caption, mask_text, self.img_ids[ann['image_id']], label
        return image, caption, self.img_ids[ann['image_id']], label, text_pseudo_label, image_pseudo_label
