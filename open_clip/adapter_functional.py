...
from torch.nn.functional import pad, linear, scaled_dot_product_attention
...
def adapter_multi_head_attention_forward(
    query: Tensor,
    key: Tensor,
    value: Tensor,
    ...
    prompt: Optional[torch.nn.Parameter] = None,
    gate1: Optional[torch.nn.Parameter] = None,
    ...
    tens_ops = (query, key, value, in_proj_weight, in_proj_bias, bias_k, bias_v, out_proj_weight, out_proj_bias, prompt)
    if has_torch_function(tens_ops):
        return handle_torch_function(
            adapter_multi_head_attention_forward,
            tens_ops,
            query,
            key,
            value,
    ...
    if not use_separate_proj_weight:
        ...
        q_pro, k_pro, v_pro = _in_projection_packed(query, prompt, prompt, in_proj_weight, in_proj_bias)
    else:
        ...
        q_pro, k_pro, v_pro = _in_projection(query, prompt, prompt, q_proj_weight, k_proj_weight, v_proj_weight, b_q, b_k, b_v)
    ...
    if bias_k is not None and bias_v is not None:
        ...
        k_pro = torch.cat([k_pro, bias_k.repeat(1, bsz, 1)])
        v = torch.cat([v, bias_v.repeat(1, bsz, 1)])
        v_pro = torch.cat([v_pro, bias_v.repeat(1, bsz, 1)])
        if attn_mask is not None:
    ...
    q_pro = q_pro.view(tgt_len, bsz * num_heads, head_dim).transpose(0, 1)
    if static_k is None:
        k = k.view(k.shape[0], bsz * num_heads, head_dim).transpose(0, 1)
        k_pro = k_pro.view(k_pro.shape[0], bsz * num_heads, head_dim).transpose(0, 1)
    else:
    ...
    if static_v is None:
        ...
        v_pro = v_pro.view(v_pro.shape[0], bsz * num_heads, head_dim).transpose(0, 1)
    else:
        # TODO finish disentangling control flow so we don't do in-projections when statics are passed
    ...
    if add_zero_attn:
        ...
        k_pro = torch.cat([k_pro, torch.zeros(zero_attn_shape, dtype=k_pro.dtype, device=k_pro.device)], dim=1)
        v = torch.cat([v, torch.zeros(zero_attn_shape, dtype=v.dtype, device=v.device)], dim=1)
        v_pro = torch.cat([v_pro, torch.zeros(zero_attn_shape, dtype=v_pro.dtype, device=v_pro.device)], dim=1)
        if attn_mask is not None:
            attn_mask = pad(attn_mask, (0, 1))
        if key_padding_mask is not None:
            key_padding_mask = pad(key_padding_mask, (0, 1))
    ...
    if need_weights:
        B, Nt, E = q.shape
        B_pro, Nt_pro, E_pro = q_pro.shape
        q_scaled = q / math.sqrt(E)
        q_pro_scaled = q_pro / math.sqrt(E_pro)
        if attn_mask is not None:
            attn_output_weights = torch.baddbmm(attn_mask, q_scaled, k.transpose(-2, -1))
        else:
            attn_output_weights = torch.bmm(q_scaled, k.transpose(-2, -1))
        attn_output_weights_pro = torch.bmm(q_scaled, k_pro.transpose(-2, -1))
        attn_output_weights_pro_scaled = attn_output_weights_pro / math.sqrt(attn_output_weights_pro.size(-1))
        attn_output_weights_pro_weights = torch.bmm(attn_output_weights_pro_scaled, attn_output_weights_pro.transpose(-2, -1))
        if attn_mask is not None:
            attn_output_weights_pro_weights = attn_output_weights_pro_weights + attn_mask
        
        attn_output_weights = (1-torch.sigmoid(gate1))*softmax(attn_output_weights, dim=-1) + torch.sigmoid(gate1)*softmax(attn_output_weights_pro_weights, dim=-1)
        attn_output_weights_pro = softmax(attn_output_weights_pro, dim=-1)
        if dropout_p > 0.0:
            attn_output_weights = dropout(attn_output_weights, p=dropout_p)
            attn_output_weights_pro = dropout(attn_output_weights_pro, p=dropout_p)

        attn_output = torch.bmm(attn_output_weights, v)
        attn_output_pro = torch.bmm(attn_output_weights_pro, v_pro)

        attn_output = attn_output.transpose(0, 1).contiguous().view(tgt_len * bsz, embed_dim)
        attn_output_pro = attn_output_pro.transpose(0, 1).contiguous().view(tgt_len * bsz, embed_dim)
        attn_output = linear(attn_output, out_proj_weight, out_proj_bias)
        attn_output_pro = linear(attn_output_pro, out_proj_weight, out_proj_bias)
        attn_output = attn_output.view(tgt_len, bsz, attn_output.size(1))
        attn_output_pro = attn_output_pro.view(tgt_len, bsz, attn_output_pro.size(1))

        # optionally average attention weights over heads
        attn_output_weights = attn_output_weights.view(bsz, num_heads, tgt_len, src_len)
        attn_output_weights_pro = attn_output_weights_pro.view(bsz, num_heads, tgt_len, pro_len)
        if average_attn_weights:
            attn_output_weights = attn_output_weights.mean(dim=1)
            attn_output_weights_pro = attn_output_weights_pro.mean(dim=1)

        if not is_batched:
            # squeeze the output if input was unbatched
            attn_output = attn_output.squeeze(1)
            attn_output_pro = attn_output_pro.squeeze(1)
            attn_output_weights = attn_output_weights.squeeze(0)
            attn_output_weights_pro = attn_output_weights_pro.squeeze(0)
        return attn_output, attn_output_weights, attn_output_pro, attn_output_weights_pro
    else:
        assert False
