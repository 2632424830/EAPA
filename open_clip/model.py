...
MLPadapter_img = nn.ModuleList([
    MLPadapter(None,hidden_size=768,layer_id=layer_id)
    for layer_id in range(12)
])
MLPadapter_text = nn.ModuleList([
    MLPadapter(None,hidden_size=512,layer_id=layer_id)
    for layer_id in range(12)
])    
...
class CLIP(nn.Module):
    def __init__(
        ...
        self.visual = _build_vision_tower(embed_dim, vision_cfg, quick_gelu, cast_dtype,MMadapter_img=MLPadapter_img,MMadapter_aux=None,modalemb=None)

        text = _build_text_tower(embed_dim, text_cfg, quick_gelu, cast_dtype,MMadapter_text=MLPadapter_text,MMadapter_aux=None,modalemb=None)
    ...
    def encode_image(self, image, normalize: bool = False):
        if self.align_before:
            features,seq_features,vis_fea = self.visual(image)
        else:
            features,seq_features = self.visual(image)
        if normalize:
            features = F.normalize(features, dim=-1)
            if self.align_before:
                vis_fea = [F.normalize(fea, dim=-1) for fea in vis_fea] 
                return features, seq_features, vis_fea
        return features, seq_features
    ...
    def encode_text(self, text, normalize: bool = False):
        ...
        if normalize:
            ...
            if self.align_before:
                ...
                return x, xs, intermediates_text
        return x, xs
